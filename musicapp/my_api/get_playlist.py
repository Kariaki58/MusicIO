...
def handle_user_form(title, artist_name, file, user):
    from musicapp.run import app
    ...
    if file and allowed_file(file.filename):
        ...
        if playlist_is_valid:
            new_playlist = Playlist(title=title, artist_name=artist_name, user_id=user)
...
def user_input_validation(title, artist_name):
    ...
    playlists = Playlist.query.filter(Playlist.artist_name==artist_name).first()
...
def insert_playlist_to_db():
    ...
    user_validate = user_input_validation(title, artist_name)
    if user_validate == "song":
        return jsonify({'message': 'song name already exist'})
    elif user_validate == "playlist":
        return jsonify({'message': 'Artist name already exist'})
    handle_user_form(title, artist_name, file, user)
